...
with st.sidebar:
    ...
    if st.button("Process Files"):
        if vector_files or graph_file or sql_files:
            ...
            if vector_files:
                from ingest import batch_ingest_vectors
                ...
                with st.spinner(f"Processing {len(vector_files)} documents for Vector DB..."):
                    try:
                        batch_ingest_vectors(temp_paths)
    ...
    with st.chat_message("assistant"):
        ...
        if final_state and "final_answer" in final_state:
            ...
            if "log_id" in final_state:
                ...
                # Mostriamo esplicitamente il ramo scelto in inglese
                arm_used = final_state.get("chosen_arm", "Unknown")
                st.write(f"🕵️‍♂️ **Routing Analysis:** The system decided to query the **`{arm_used.upper()}`** node for this question.")
                
                st.write("*Did the router select the correct database for your question?*")
                col1, col2, _ = st.columns([1.5, 1.5, 7])
                with col1:
                    st.button("✅ Correct Route", on_click=handle_feedback, args=(1,), key=f"up_{log_id}")
                with col2:
                    st.button("❌ Incorrect Route", on_click=handle_feedback, args=(0,), key=f"down_{log_id}")
